-- Improved Balloon Pump Game Function with Randomness and Better Mechanics
...
BEGIN
  ...
  IF new_pressure_val >= pop_threshold_val THEN
    ...
    IF is_test THEN
      ...
      VALUES (user_address, 'balloon_pop_win', CAST(winner_reward_val AS TEXT), 1, true);
    END IF;
    
    -- Create new round with incremented ID
    INSERT INTO rounds_cache (round_id, status, pressure, pot, last1, last2, last3, created_at, updated_at)
    VALUES (
      (SELECT COALESCE(MAX(round_id), 0) + 1 FROM rounds_cache),
      'active',
      '0',
      '0',
      NULL,
    ...
    -- Update current round to ended status
  ...
  IF is_test THEN
    INSERT INTO token_transactions (user_id, transaction_type, amount, round_id, is_test_mode)
    VALUES (user_address, 'pump', pump_amount, 1, true);
  END IF;
